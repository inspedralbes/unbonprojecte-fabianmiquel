...
export default {
  data() {
    return { animales: [], animalesFiltrados: [], filtro: '' };
  },
  mounted() {
  ...
  methods: {
    async obtenerAnimales() {
      try {
        ...
        this.animales = data; this.animalesFiltrados = data;
      }
    ...
    filtrarAnimales() {
      if (this.filtro === '') {
        ...
        this.animalesFiltrados = this.animales.filter(animal => animal.nombre.toLowerCase().includes(this.filtro.toLowerCase()) || animal.caracteristica.toLowerCase().includes(this.filtro.toLowerCase()));
      }
    }
  }
}; </script>
